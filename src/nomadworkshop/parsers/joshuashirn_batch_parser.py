...
from nomadworkshop.schema_packages.joshuashirn_package import (
    joshuashirn_Batch,
    joshuashirn_Cleaning,
    joshuashirn_Evaporation,
    joshuashirn_Inkjet_Printing,
    joshuashirn_LaserScribing,
    joshuashirn_Process,
    joshuashirn_Sample,
    joshuashirn_SlotDieCoating,
    joshuashirn_SpinCoating,
    joshuashirn_Sputtering,
    joshuashirn_Substrate,
    joshuashirn_AtomicLayerDeposition,
    ProcessParameter,
)
...
class RawjoshuashirnExperiment(EntryData):
...
class joshuashirnExperimentParser(MatchingParser):
    def is_mainfile(
        self,
        filename: str,
        mime: str,
    ...
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        archives = [map_batch(sample_ids, batch_id, upload_id, joshuashirn_Batch)]
        substrates = []
        substrates_col = [
            'Sample dimension',
            'Sample area [cm^2]',
        ...
        for i, sub in df['Experiment Info'][substrates_col].drop_duplicates().iterrows():
            if pd.isna(sub).all():
                continue
            substrates.append((f'{i}_substrate', sub, map_substrate(sub, joshuashirn_Substrate)))
        ...
        for i, row in df['Experiment Info'].iterrows():
            ...
            archives.append(map_basic_sample(row, substrate_name, upload_id, joshuashirn_Sample))
        ...
        for i, col in enumerate(df.columns.get_level_values(0).unique()):
            ...
            for j, row in df_dropped.iterrows():
                ...
                if 'Cleaning' in col:
                    archives.append(map_cleaning(i, j, lab_ids, row, upload_id, joshuashirn_Cleaning))

                if 'Laser Scribing' in col:
                    archives.append(map_laser_scribing(i, j, lab_ids, row, upload_id, joshuashirn_LaserScribing))

                if 'Generic Process' in col:  # move up
                    generic_process = map_generic(i, j, lab_ids, row, upload_id, joshuashirn_Process)
                ...
                if 'Evaporation' in col:
                    ...
                    if 'Co-Evaporation' in col:
                        coevap = True
                    archives.append(
                        map_evaporation(i, j, lab_ids, row, upload_id, joshuashirn_Evaporation, coevap)
                    )

                if 'Spin Coating' in col:
                    archives.append(map_spin_coating(i, j, lab_ids, row, upload_id, joshuashirn_SpinCoating))

                if 'Slot Die Coating' in col:
                    archives.append(map_sdc(i, j, lab_ids, row, upload_id, joshuashirn_SlotDieCoating))

                if 'Sputtering' in col:
                    archives.append(map_sputtering(i, j, lab_ids, row, upload_id, joshuashirn_Sputtering))

                if 'Inkjet Printing' in col:
                    archives.append(
                        map_inkjet_printing(i, j, lab_ids, row, upload_id, joshuashirn_Inkjet_Printing)
                    )

                if 'ALD' in col:
                    archives.append(
                        map_atomic_layer_deposition(
                            i, j, lab_ids, row, upload_id, joshuashirn_AtomicLayerDeposition)
                    )
        ...
        archive.data = RawjoshuashirnExperiment(processed_archive=refs)
