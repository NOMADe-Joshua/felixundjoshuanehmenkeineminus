...
from nomadworkshop.schema_packages.joshuashirn_package import (
    joshuashirn_EQEmeasurement,
    joshuashirn_JVmeasurement,
    joshuashirn_Measurement,
    joshuashirn_SimpleMPPTracking,
)
...
class RawFilejoshuashirn(EntryData):
    processed_archive = Quantity(
        type=Activity,
    )


class joshuashirnParser(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        entry = joshuashirn_Measurement()

        if measurment_type == 'jv':
            entry = joshuashirn_JVmeasurement()
        if measurment_type == 'eqe':
            entry = joshuashirn_EQEmeasurement()
        if measurment_type == 'mppt':
            entry = joshuashirn_SimpleMPPTracking()
        ...
        archive.data = RawFilejoshuashirn(processed_archive=get_reference(archive.metadata.upload_id, eid))
